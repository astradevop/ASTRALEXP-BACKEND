...
import { chatAPI, expensesAPI, paymentsAPI } from '../services/api';
...
export default function ChatScreen({ navigation }) {
  ...
  const loadPaymentMethods = async () => {
    try {
      ...
      setPayMethods(res.data.results || res.data);
    } catch {}
  };
  ...
  const send = async (photoBase64 = null, photoUri = null, textOverride = null, explicitContext = null) => {
    ...
    try {
      ...
      if (is_complete) {
        ...
        if (p?.missing_fields?.includes('payment_method') && payMethods.length) {
          pushChips(payMethods.slice(0, 4).map(pm => {
            const balStr = pm.balance != null ? `${currSym}${parseFloat(pm.balance).toFixed(0)}` : '';
            const labelStr = balStr ? `${pm.name} (${balStr})` : pm.name;
            const isInsufficient = pm.balance != null && parseFloat(pm.balance) < (p.amount || 0);

            return {
              label: isInsufficient ? `🚫 ${labelStr}` : labelStr,
              onPress: isInsufficient 
                ? () => Alert.alert('Insufficient Balance', `You only have ${balStr} in ${pm.name}.`) 
                : () => send(null, null, `via ${pm.name}`, { parsed: p, is_complete_action: false }),
            };
          }));
        }
      }
  ...
  const fullAvatarUrl = user?.avatar && !user.avatar.startsWith('file:') && !user.avatar.startsWith('data:') && !user.avatar.startsWith('http')
    ? `http://10.91.217.224:8000${user.avatar}`
    : user?.avatar;
